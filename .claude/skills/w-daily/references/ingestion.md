...
- Entity matching: `.claude/rules/entity-matching.md` (auto-loaded)
- Email-specific rules (pulling, Power Automate format, parsing, email frontmatter): `references/ingestion-email.md`
- Email preprocessing (cleaning, relevance scoring, threading): `references/email-preprocessing.md`
...
- `_email-attachments/` subdirectory contents: email attachments staged by `Pull-Emails.ps1`, claimed by their parent email via the receive-second stamp (see `references/ingestion-email.md`). Never classify these as documents. Directories are already skipped by the `is_file()` filter in `classify-inbox.py`, so this needs no special-casing, but a loose attachment sitting in `00-Inbox/` root WOULD be routed to `08-Reference/` as an orphan reference note
