...
└── .claude/                     # Claude Code config
    ├── SYSTEM.md                # ← This file
    ├── rules/                   # Rule definitions (auto-loaded every session)
    └── skills/                  # Skill definitions
        └── w-daily/references/  # Ingestion procedures, read on demand (NOT auto-loaded)
```
...
| `audit-project-metadata.py` | On-demand (report, `--fix`, `--json`)             | Auditor for the `project:` frontmatter field, which no index validates (`entity-registry.json` holds people, not projects, so a renamed project leaves silently broken tags). **With no configuration it validates link integrity**: every `project:` must point at a real `03-Projects/*.md` note, reported as `UNKNOWN-TARGET`. Fill in the `PROJECT SIGNAL CONFIG` block to also backfill missing tags and flag misfiled notes. The resolver scores signal **dominance**, not keyword presence: anchor hits are counted against hard/soft out-of-domain guards, so one stray mention of another domain cannot veto a note and a bare keyword cannot claim one. Notes matching 3+ top-level areas are cross-cutting and always flagged, never auto-tagged. `--fix` applies `MISMATCH`, `MISSING-HIGH-CONFIDENCE` and `EMPTY/MALFORMED` only; `UNCERTAIN` and `UNKNOWN-TARGET` are never written. Never creates a project note, never edits outside `05-Interactions/` |
| `vault-health.py`           | On-demand / scheduled (cron)                      | Detect-only maintenance report. Original checks: overdue open actions, stale `status: stub` people (older than `--stale-stub-days`, default 90, measured from the stub's `created:` field so a later edit cannot reset the clock), archive-candidate interactions (older than 2 quarters with no open action), ghost ingest-log entries. Coherence checks (single content-tree pass, infra dirs excluded): broken wikilinks (a `[[target]]` that is neither a note stem nor a known `entity-registry.json` entry, case-insensitive so casing drift stays with `audit-link-casing.py`; attachment/`.base` embeds, punctuation-only links, and any `[[target]]` inside a code fence or code span are skipped), orphaned entity notes (`04-People`/`03-Projects` with no inbound links, ignoring notes younger than `--orphan-grace-days`, default 14, and anything under `04-People/_archived/`, which is where a dormant person is deliberately put), and unresolved-entity notes (non-empty `unresolved-entities:` frontmatter). Writes `_db/maintenance-todo.md` (gitignored) plus a stderr summary; never fixes (detect-then-delegate). `--json` available |
...
| ------------------------------------------------------------- | --------------------- | ---------------------------------------- |
| Note-writing agents (transcript, email, doc)                  | `sonnet` (`haiku` for low-stakes transcripts) | High volume, rule-driven, cost efficient |
| Synthesis agents (review-agent, 1on1-prep)                    | `opus`, set on the Agent call | Complex analysis, writing quality        |
| w-project-status, w-prep                                      | `opus`, frontmatter   | Judgment-heavy, user-facing synthesis    |
| Master commands (w-setup, w-daily, w-review, w-1on1, w-task-audit) | Current session model | Orchestration only                       |

Every routing value above is a floating **alias**, never a pinned model ID. An alias follows whatever the current generation of that tier is, so a template that sits unused for months still routes correctly on first run. A pinned `claude-opus-4-6`-style ID does not, and it rots silently.

`review-agent` and `1on1-prep` carry no frontmatter `model:` on purpose. Both are dispatched under `subagent_type: "general-purpose"`, which does not honor skill frontmatter, so the pin belongs on the Agent call in `w-review` / `w-1on1`. A frontmatter copy would only read as though it were doing something.
